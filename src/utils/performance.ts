...
export function createPerformanceMonitor(): PerformanceMonitor {
	return Object.freeze({
		start(): PerformanceTimer {
			...
			const cpuUsage =
				timer.cpuStart !== undefined && cpuEnd !== undefined
					? cpuEnd - timer.cpuStart
					: undefined;
			...
			async (progress, token) => {
				try {
					const progressReporter: ProgressReporter = Object.freeze({
						report(increment: number, message?: string): void {
							const reportData: { increment: number; message?: string } = {
								increment,
							};
...
export function formatThroughput(throughput: number): string {
	if (throughput < 1) return `${(throughput * 1000).toFixed(0)} URLs/min`;
	if (throughput < 60) return `${throughput.toFixed(2)} URLs/sec`;
